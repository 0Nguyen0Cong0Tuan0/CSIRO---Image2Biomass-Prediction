...
def post_process_biomass(df_preds):
    ...
    # 1. Reconciliation (Existing Logic - KEEP THIS)
    ...
    # 2. Zero-Clamping (NEW LOGIC)
    # If the reconciled value is very small, force it to zero.
    # Thresholds are hyperparameters you can tune. 
    CLOVER_THRESHOLD = 1.25  # From convnext notebook
    DEAD_THRESHOLD = 1.0
    
    # Apply thresholds to the reconciled array
    # Indices: 0=Green, 1=Clover, 2=Dead, 3=GDM, 4=Total
    clover_col_idx = 1
    dead_col_idx = 2
    
    Y_reconciled[:, clover_col_idx] = np.where(
        Y_reconciled[:, clover_col_idx] < CLOVER_THRESHOLD, 
        0.0, 
        Y_reconciled[:, clover_col_idx]
    )
    Y_reconciled[:, dead_col_idx] = np.where(
        Y_reconciled[:, dead_col_idx] < DEAD_THRESHOLD, 
        0.0, 
        Y_reconciled[:, dead_col_idx]
    )
    
    # 3. Re-calculate sums after clamping to ensure consistency
    Y_reconciled[:, 3] = Y_reconciled[:, 0] + Y_reconciled[:, 1] # GDM = Green + Clover
    Y_reconciled[:, 4] = Y_reconciled[:, 3] + Y_reconciled[:, 2] # Total = GDM + Dead
...
def ensemble_with_jitter(predictions_list, jitter_strength=0.1):
    """
    predictions_list: List of arrays [pred_hb, pred_gb, ...]
    jitter_strength: How much to deviate from equal weights. 
                     0.0 = equal weights, 1.0 = highly random.
    """
    ...
    # 2. Add random noise centered around 0
    noise = np.random.normal(loc=0.0, scale=jitter_strength * base_weight, size=n_models)
    weights = weights + noise
    
    # 3. Clip to ensure positive weights and re-normalize to sum to 1
    weights = np.maximum(weights, 0.01) # ensure no negative weights
...
def main():
    ...
    # Run this once to get your prediction
