...
# ============================================================================
# SECTION 1: CONFIGURATION & ENHANCED CONSTANTS
# ============================================================================
...
class Config:
    ...
    # Enhanced Physics Parameters (from V5)
    USE_MINT = True
...
# ============================================================================
# SECTION 2: ENHANCED PHYSICS CORE (MinT & HCL)
# ============================================================================

def build_hierarchy_matrix():
    """
    Construct summation matrix S mapping base components to all targets
    Equation: y = S * b where b = [Green, Dead, Clover]^T
    """
    # Columns: Green, Dead, Clover
    # Rows: Dry_Clover_g, Dry_Dead_g, Dry_Green_g, Dry_Total_g, GDM_g (Sorted alphabetically match TARGET_NAMES)
    # TARGET_NAMES = ['Dry_Clover_g', 'Dry_Dead_g', 'Dry_Green_g', 'Dry_Total_g', 'GDM_g']
    ...
    S = np.array([
        ...
        [0, 1, 0],  # Dry_Dead_g
        [1, 0, 0],  # Dry_Green_g
        [1, 1, 1],  # Dry_Total_g = Green + Dead + Clover
        [1, 0, 1],  # GDM_g = Green + Clover
    ], dtype=np.float32)
    return S

def mint_reconciliation(predictions_raw, error_covariance=None, S=None):
    """
    Minimum Trace (MinT) Reconciliation - V5 Enhancement
    Projects incoherent forecasts onto coherent manifold using error covariance.
    """
    if S is None:
    ...
    # predictions_raw shape: (N, 5), P shape: (5, 5)
    predictions_reconciled = (P @ predictions_raw.T).T
    
    return predictions_reconciled

def post_process_biomass_v5(df_preds, use_mint=True):
    """
    Enhanced V5 Post-Processing: MinT + Threshold Clamping
    """
    # Ensure correct column order matching TARGET_NAMES for the matrix S
    # TARGET_NAMES = ['Dry_Clover_g', 'Dry_Dead_g', 'Dry_Green_g', 'Dry_Total_g', 'GDM_g']
    Y = df_preds[TARGET_NAMES].values
    
    if use_mint:
        # We use Identity covariance here as a robust default (OLS)
        # For even better results, one could pass the validation error covariance
        Y_reconciled = mint_reconciliation(Y, error_covariance=np.eye(5))
    else:
        Y_reconciled = Y
    ...
    # 2. Zero-Clamping (V5 Implementation Guide)
    # Thresholds: Clover < 1.25 -> 0, Dead < 1.0 -> 0
    
    clover_idx = TARGET_NAMES.index("Dry_Clover_g")
    dead_idx = TARGET_NAMES.index("Dry_Dead_g")
    green_idx = TARGET_NAMES.index("Dry_Green_g")
    gdm_idx = TARGET_NAMES.index("GDM_g")
    total_idx = TARGET_NAMES.index("Dry_Total_g")
    
    CLOVER_THRESHOLD = 1.25
    DEAD_THRESHOLD = 1.0
    
    Y_reconciled[:, clover_idx] = np.where(Y_reconciled[:, clover_idx] < CLOVER_THRESHOLD, 0.0, Y_reconciled[:, clover_idx])
    Y_reconciled[:, dead_idx] = np.where(Y_reconciled[:, dead_idx] < DEAD_THRESHOLD, 0.0, Y_reconciled[:, dead_idx])
    
    # 3. Re-enforce Summation Constraints after clamping
    # GDM = Green + Clover
    Y_reconciled[:, gdm_idx] = Y_reconciled[:, green_idx] + Y_reconciled[:, clover_idx]
    # Total = GDM + Dead
    Y_reconciled[:, total_idx] = Y_reconciled[:, gdm_idx] + Y_reconciled[:, dead_idx]
    
    # 4. Cap at Max Values (Safety)
    for i, target in enumerate(TARGET_NAMES):
         Y_reconciled[:, i] = np.minimum(Y_reconciled[:, i], TARGET_MAX[target])

    df_out = df_preds.copy()
    df_out[TARGET_NAMES] = Y_reconciled
    return df_out
...
def generate_semantic_features(image_embeddings, model_path):
    ...
    concept_groups = {
        ...
        "species_specific": ["lucerne alfalfa", "ryegrass", "fescue", "chicory", "plantain"],
        "texture": ["leafy texture", "stemmy texture", "flowering heads"],
        "color_nuance": ["yellow dry grass", "brown dead matter", "lime green new growth"]
    }
    concept_vectors = {}
    with torch.no_grad():
...
# ============================================================================
# SECTION 4: ENHANCED VALIDATION PIPELINE
# ============================================================================

def competition_metric_detailed(y_true, y_pred) -> Tuple[float, float]:
    """
    Returns (Weighted R2, HCL Penalty)
    """
    y_weighted = 0
    ...
    green = y_pred[:, green_idx]
    dead = y_pred[:, dead_idx]
    clover = y_pred[:, clover_idx]
    gdm = y_pred[:, gdm_idx]
    total = y_pred[:, total_idx]

    total_violation = np.mean((total - (green + dead + clover))**2)
    gdm_violation = np.mean((gdm - (green + clover))**2)
    hcl = total_violation + gdm_violation
...
def cross_validate(model, train_data, test_data, feature_engine, semantic_train=None, semantic_test=None, target_transform='max', seed=42):
    ...
    for fold in range(n_splits):
        ...
        # --- ENHANCED RECONCILIATION IN CV LOOP ---
        # Apply MinT to OOF predictions to get accurate CV metrics
        fold_valid_df = pd.DataFrame(fold_valid_pred, columns=TARGET_NAMES)
        fold_valid_rec = post_process_biomass_v5(fold_valid_df, use_mint=True)
        fold_valid_pred = fold_valid_rec[TARGET_NAMES].values
        
        # Apply MinT to Test predictions per fold
        fold_test_df = pd.DataFrame(fold_test_pred, columns=TARGET_NAMES)
        fold_test_rec = post_process_biomass_v5(fold_test_df, use_mint=True)
        fold_test_pred = fold_test_rec[TARGET_NAMES].values
        
        y_pred.loc[val_idx] = fold_valid_pred
        y_pred_test += fold_test_pred / n_splits

        # Metrics
        f_r2, f_hcl = competition_metric_detailed(y_valid, fold_valid_pred)
        fold_r2_list.append(f_r2)
        fold_hcl_list.append(f_hcl)
        print(f"  Fold {fold} | R2: {f_r2:.5f} | HCL: {f_hcl:.6f}")

    full_cv_r2, full_cv_hcl = competition_metric_detailed(y_true.values, y_pred.values)
    print(f"Full CV Score: {full_cv_r2:.6f} | Full CV HCL: {full_cv_hcl:.6f}")
    return y_pred.values, y_pred_test

def compare_results(oof, train_data):
    y_oof_df = pd.DataFrame(oof, columns=TARGET_NAMES)
    raw_r2, raw_hcl = competition_metric_detailed(train_data[TARGET_NAMES].values, y_oof_df.values)
    print(f"Raw CV Score: {raw_r2:.6f} | HCL: {raw_hcl:.6f}")
    
    y_oof_proc = post_process_biomass_v5(y_oof_df, use_mint=True)
    proc_r2, proc_hcl = competition_metric_detailed(train_data[TARGET_NAMES].values, y_oof_proc[TARGET_NAMES].values)
    print(f"Processed CV Score: {proc_r2:.6f} | HCL: {proc_hcl:.6f}")
    print(f"Improvement: {proc_r2 - raw_r2:.6f}")
...
def ensemble_with_jitter(predictions_list, jitter_strength=0.1):
    """
    Enhanced with uncertainty weighting concept from V5:
    Instead of pure random jitter, we could weight by inverse variance.
    For now, keeping jitter but normalizing strictly.
    """
    n_models = len(predictions_list)
    base_weight = 1.0 / n_models
    weights = np.full(n_models, base_weight)
    noise = np.random.normal(loc=0.0, scale=jitter_strength * base_weight, size=n_models)
    weights = weights + noise
    weights = np.maximum(weights, 0.01)
    weights = weights / np.sum(weights)
    print("Generated Ensemble Weights:", np.round(weights, 3))
...
# ============================================================================
# SECTION 5: DEEP LEARNING ARCHITECTURES (Preserved for Checkpoint Compatibility)
# ============================================================================
...
def run_inference(test_df: pd.DataFrame, image_dir: str) -> np.ndarray:
    ...
    if len(models) == 0:
        raise RuntimeError("No available checkpoint found!")
    if INF_CFG.USE_TTA:
...
def create_submission(final_pred: np.ndarray, test_long: pd.DataFrame, test_unique: pd.DataFrame) -> pd.DataFrame:
    # 1. Map predictions to DataFrame
    wide = pd.DataFrame({"image_path": test_unique["image_path"]})
    # Our DL model returns [green, dead, clover, gdm, total]
    # TARGET_NAMES order: ['Dry_Clover_g', 'Dry_Dead_g', 'Dry_Green_g', 'Dry_Total_g', 'GDM_g']
    # Pack5 returns: [green, dead, clover, gdm, total]
    # Need to map carefully
    
    wide["Dry_Green_g"] = final_pred[:, 0]
    wide["Dry_Dead_g"] = final_pred[:, 1]
    wide["Dry_Clover_g"] = final_pred[:, 2]
    wide["GDM_g"] = final_pred[:, 3]
    wide["Dry_Total_g"] = final_pred[:, 4]
    
    # 2. Apply V5 Post-Processing (MinT + Thresholds)
    wide = post_process_biomass_v5(wide, use_mint=True)
...
def parse_args():
    parser = argparse.ArgumentParser(description="CSIRO v5 CrossPVT T2T Mamba Inference")
...
def create_submission_mvp(final_preds, test_df, unique_df):
    ...
    # order returned by DL model: green, dead, clover, gdm, total
...
def main():
    ...
    # Stage 1: The 'Semantic Embedding' pipeline (classical ML)
    print("\n" + "="*50)
    print("Stage 1: SigLIP/Ensemble Model (Enhanced)")
    print("="*50)
    ...
    # Model Ensemble with SigLIP features
    ...
    print("\nTraining HistGB...")
    oof_hb, pred_test_hb = cross_validate(HistGradientBoostingRegressor(loss='poisson', max_iter=300), train_siglip_df, test_siglip_df, feature_engine=feat_engine, semantic_train=sem_train_full, semantic_test=sem_test_full)
    compare_results(oof_hb, train_siglip_df)
    
    print("\nTraining CatBoost...")
    oof_cat, pred_test_cat = cross_validate(CatBoostRegressor(loss_function='Tweedie:variance_power=1.5', verbose=0), train_siglip_df, test_siglip_df, feature_engine=feat_engine, semantic_train=sem_train_full, semantic_test=sem_test_full)
    compare_results(oof_cat, train_siglip_df)
    
    print("\nTraining LGBM...")
    oof_lgbm, pred_test_lgbm = cross_validate(LGBMRegressor(objective='tweedie', tweedie_variance_power=1.5, n_estimators=1000, learning_rate=0.03, verbose=-1), train_siglip_df, test_siglip_df, feature_engine=feat_engine, semantic_train=sem_train_full, semantic_test=sem_test_full, target_transform='log')
    compare_results(oof_lgbm, train_siglip_df)
    

    # --- Ensemble and Post-Process Stage 1 ---
    all_preds = [pred_test_hb, pred_test_gb, pred_test_cat, pred_test_lgbm]

    # Weighted ensemble with jitter
    pred_test = ensemble_with_jitter(all_preds, jitter_strength=0.2)
    test_df[TARGET_NAMES] = pred_test

    # Apply V5 Post-Processing to Stage 1 Result
    test_df = post_process_biomass_v5(test_df, use_mint=True)
    sub_df = melt_table(test_df)
    sub_df[['sample_id', 'target']].to_csv("submission1.csv", index=False)
    
    print("\n" + "="*50)
    print("Stage 2: DINO Model 1")
    print("="*50)
    run_dino_inference()
    
    print("\n" + "="*50)
    print("Stage 3: DINO Model 2")
    print("="*50)
    run_mvp_inference()
    
    print("\n" + "="*50)
    print("Final Ensemble & Reconciliation")
    print("="*50)
    ...
    # Note: Because each individual submission has already been reconciled via post_process_biomass_v5
    # inside their respective functions, the weighted average *should* be close to consistent.
    # However, for maximum rigor, we reconcile the FINAL weighted average once more.
    
    # Need to pivot back to wide format to reconcile
    final_submission = merged[['sample_id', 'target', 'image_path_1', 'target_name']] # Assuming image_path_1 exists
    # Recover image_path from sample_id if needed, but here we can just rely on the fact
    # that we processed them independently. 
    # Actually, let's just save. Reconciling the final mix requires pivot/melt which is complex here.
    # Since inputs are MinT-reconciled, the linear combination is also largely consistent.
