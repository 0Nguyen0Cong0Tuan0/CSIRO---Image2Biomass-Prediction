...
class Config:
    ...
    # --- Integrated Physics Parameters (from Alchemy V5) ---
    USE_MINT: bool = True           # Use Minimum Trace Reconciliation
    MINT_REGULARIZATION: float = 1e-8
    TWEEDIE_POWER: float = 1.5      # For zero-inflated biomass modeling
...
# ============================================================================
# SECTION 2: PHYSICS CORE (Integrated from Alchemy V5)
# ============================================================================

def build_hierarchy_matrix():
    """
    Constructs the summation matrix S mapping base components to all targets.
    Formula: y_all = S * y_base
    Base components: [Green, Dead, Clover] (Indices in sorted TARGET_NAMES)
    Targets: ['Dry_Clover_g', 'Dry_Dead_g', 'Dry_Green_g', 'Dry_Total_g', 'GDM_g']
    """
    # Columns correspond to Base: [Dry_Clover_g (idx 0), Dry_Dead_g (idx 1), Dry_Green_g (idx 2)]
    # Rows correspond to All: [Clover, Dead, Green, Total, GDM]
    S = np.array([
        [1, 0, 0],  # Dry_Clover_g = 1*Clover
        [0, 1, 0],  # Dry_Dead_g   = 1*Dead
        [0, 0, 1],  # Dry_Green_g  = 1*Green
        [1, 1, 1],  # Dry_Total_g  = Clover + Dead + Green
        [1, 0, 1],  # GDM_g        = Clover + Green
    ], dtype=np.float32)
    return S
...
def mint_reconciliation(predictions_raw, error_covariance=None, S=None):
    ...
    Projects incoherent forecasts onto the coherent manifold defined by S.
    """
    if S is None:
        S = build_hierarchy_matrix()
    
    # If no covariance provided, use Identity (OLS reconciliation)
    # Ideally, pass the validation error covariance matrix here.
    ...
    # Projection Matrix P = S(S^T W^-1 S)^-1 S^T W^-1
    S_T = S.T
    middle = np.linalg.inv(S_T @ W_h_inv @ S)
    P = S @ middle @ S_T @ W_h_inv
    
    # Apply: Y_rec = P * Y_raw
    return (P @ predictions_raw.T).T

def post_process_biomass(df_preds):
    """
    Upgraded Post-Processing using MinT + Thresholds (Integrated from V5)
    """
    # Ensure column order matches TARGET_NAMES
    Y = df_preds[TARGET_NAMES].values
    
    # 1. MinT Reconciliation (Replaces manual matrix math)
    if cfg.USE_MINT:
        Y_reconciled = mint_reconciliation(Y, error_covariance=np.eye(5))
    else:
        Y_reconciled = Y

    # 2. Non-negativity & Zero-Clamping (Research Thresholds)
    # Indices in TARGET_NAMES: ['Dry_Clover_g', 'Dry_Dead_g', 'Dry_Green_g', 'Dry_Total_g', 'GDM_g']
    clover_idx, dead_idx = 0, 1
    green_idx, total_idx, gdm_idx = 2, 3, 4
    
    CLOVER_THRESHOLD = 1.25
    DEAD_THRESHOLD = 1.0
    
    Y_reconciled = np.maximum(Y_reconciled, 0) # Global non-negative
    
    # Hard clamping for small noise
    ...
    # 3. Re-enforce consistency after clamping
...
# ============================================================================
# SECTION 3: LOSS FUNCTIONS & ALCHEMY ARCHITECTURE
# ============================================================================

class TweedieLoss(nn.Module):
    """Integrated from Alchemy: Handles zero-inflated biomass distribution"""
...
class CompositionalHead(nn.Module):
    """
    Integrated from Alchemy Idea #1:
    Predicts Total + Ratios instead of independent values.
    Can be used to augment the existing Mamba architecture in future iterations.
    """
    def __init__(self, input_dim: int, dropout: float = 0.2):
        ...
        self.magnitude_head = nn.Sequential(
            ...
            nn.Dropout(dropout), nn.Linear(hidden, 1), nn.Softplus(beta=1.0)
        )
        self.ratio_head = nn.Sequential(
            nn.LayerNorm(input_dim), nn.Linear(input_dim, hidden), nn.GELU(),
            nn.Dropout(dropout), nn.Linear(hidden, 3) # [Green, Clover, Dead]
        )
        
    def forward(self, features):
        total = self.magnitude_head(features)
        ratios = F.softmax(self.ratio_head(features), dim=-1)
        green = total * ratios[:, 0:1]
        clover = total * ratios[:, 1:2]
        dead = total * ratios[:, 2:3]
        gdm = green + clover
        return {'total': total, 'green': green, 'clover': clover, 'dead': dead, 'gdm': gdm}

# ============================================================================
# SECTION 4: DATA UTILITIES
# ============================================================================
...
# ============================================================================
# SECTION 5: ML ENSEMBLE PIPELINE
# ============================================================================
...
def cross_validate(model, train_data, test_data, feature_engine, semantic_train=None, semantic_test=None, target_transform='max', seed=42):
    ...
    for fold in range(n_splits):
        ...
        # --- Integrated: Valid Set Reconciliation for accurate CV ---
        fold_valid_df = pd.DataFrame(fold_valid_pred, columns=TARGET_NAMES)
        fold_valid_df = post_process_biomass(fold_valid_df) # Applies MinT
        y_pred.loc[val_idx] = fold_valid_df[TARGET_NAMES].values
        
        # Test fold is averaged
        y_pred_test += fold_test_pred / n_splits
        
    full_cv = competition_metric(y_true.values, y_pred.values)
    print(f"Full CV Score (Reconciled): {full_cv:.6f}")
    return y_pred.values, y_pred_test

def compare_results(oof, train_data):
    y_oof_df = pd.DataFrame(oof, columns=TARGET_NAMES)
    # Note: OOF passed here is already MinT reconciled inside cross_validate loop
    proc_score = competition_metric(train_data[TARGET_NAMES].values, y_oof_df.values)
    print(f"Verified CV Score: {proc_score:.6f}")
...
def ensemble_with_jitter(predictions_list, jitter_strength=0.1):
    ...
    print("Generated Weights:", np.round(weights, 3))
...
# ============================================================================
# SECTION 6: DL MODEL ARCHITECTURES (Baseline Mamba + DINO)
# ============================================================================
...
class CrossPVT_T2T_MambaDINO(nn.Module):
    ...
    def _build_dino_backbone(self):
        ...
        raise RuntimeError(f"Unable to create DINO backbone. Last error: {last_err}")
...
def create_submission(final_pred: np.ndarray, test_long: pd.DataFrame, test_unique: pd.DataFrame) -> pd.DataFrame:
    ...
    # Map array to columns based on target order
    ...
    # --- Integrated: Post-Process with MinT ---
...
def load_model(checkpoint_path):
    ...
    try: raw_state = torch.load(checkpoint_path, map_location=INF_CFG.DEVICE, weights_only=False)
    ...
    for backbone in backbones:
        try:
            ...
            if len(missing) == 0:
                model.to(INF_CFG.DEVICE); model.eval(); return model
...
def predict_one_view_mvp(models, loader):
    preds = []; use_amp = INF_CFG.DEVICE.type == "cuda"
...
def main():
    ...
    print("\n" + "="*50)
    print("Stage 1: SigLIP/Ensemble Model (Physics-Aware)")
    print("="*50)
    ...
    print("\nTraining Ensemble (Using V5 Configs)...")
    # Integrated: Passed config parameters to models where applicable
    ...
    oof_cat, pred_test_cat = cross_validate(CatBoostRegressor(loss_function=f'Tweedie:variance_power={cfg.TWEEDIE_POWER}', verbose=0), train_siglip_df, test_siglip_df, feature_engine=feat_engine, semantic_train=sem_train_full, semantic_test=sem_test_full)
    compare_results(oof_cat, train_siglip_df)
    
    oof_lgbm, pred_test_lgbm = cross_validate(LGBMRegressor(objective='tweedie', tweedie_variance_power=cfg.TWEEDIE_POWER, n_estimators=1000, learning_rate=0.03, verbose=-1), train_siglip_df, test_siglip_df, feature_engine=feat_engine, semantic_train=sem_train_full, semantic_test=sem_test_full, target_transform='log')
    ...
    print("\n" + "="*50)
    print("Stage 2: DINO Model 1 (Mamba/PVT Backbone)")
    print("="*50)
    run_dino_inference()
    
    print("\n" + "="*50)
    print("Stage 3: DINO Model 2 (Tiled FiLM Backbone)")
    print("="*50)
    run_mvp_inference()
