**CSIRO · Research Code Competition**
...
***Terms***
- $y_j$ $\rightarrow$ ground-truth value for data point $j$
- $\hat{y}_j$ $\rightarrow$ model prediction for data point $j$
- $w_j$ $\rightarrow$ per-row weight based on target type
- $\bar{y}_w$ $\rightarrow$ global weighted mean of all ground-truth values
## **Submission File**
Submit a CSV in long format with exactly two columns:

- `sample_id` $\rightarrow$ ID constructed from image ID and target_name pair.
- `target` $\rightarrow$ Your predicted biomass value (grams) for that sample_id (float).
